...
void parseTag(int sockFD) {
		...
		if (g_totalTagCount == 2) {
			while (getNextAttr(sockFD, attrName, attrValue)) {
				if (strcmp(attrName, attributes[attr_roomId]) == 0) {
					...
					g_roomName = (char *)malloc((roomIdLen + 1) * sizeof(char));
	...
	case tag_data:
		if (getNextAttr(sockFD, attrName, attrValue)) {
			if (strcmp(attrName, attributes[attr_class]) == 0) {
				int attr_class = getMatchingStringID(attrValue, classes, CLASS_COUNT);
				switch (attr_class) {
				case class_memento:
    ...
    case tag_board:
        ...
        case tag_field:
        {
            ...
            if (fieldType != 3) {
                BoardManager::SetBoardValue(g_state.board, x, y, fieldType);
            }
        }
            break;
        case tag_lastMove:
